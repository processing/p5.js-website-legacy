description__0 = Esse é o construtor de p5 instance (instância p5). 
...
description__2 = Um sketch p5 pode rodar em "global" ou "instance" modo: "global" - todas as propriedades e métodos estão atrelados à janela "instance" - todas as propriedades e métodos estão atrelados ao este objeto p5
...
describeElement__description__0 = Esta função cria uma descrição acessível a leitores de tela para elementos - formas ou grupos de formas que só tem signficado juntas — na canvas. O primeiro parâmetro deve ser o nome do elemento. O segundo parâmetro deve ser uma string com a descrição do elemento. O terceiro parâmetro é opcional. Se especificado, ele determina como a descrição do elemento será mostrada.
...
lerpColor__description__0 = Mistura duas cores para encontrar uma terceira entre as duas. O parâmetro amt é a quantidade para interpolar entre dois valores, onde 0 é igual à primeira cor, e 1 é igual à segunda — 0.1 é muito próximo da primeira cor, 0.5 é a média, etc. Um valor menor que 0 será considerado igual a 0. Da mesma maneira, valores acima de 1 serão considerados iguais a 1. Esse comportamento é diferente de <a href="#/p5/lerp">lerp()</a>, mas necessário para evitar números fora do intervalo que podem produzir cores e resultados inesperados.
lerpColor__description__1 = A maneira que as cores são interpoladas depende do modo de cor em uso (<a href="#/p5/colorMode">colorMode()</a>).
...
bezierDetail__description__1 = Leve em consideração que essa função só é útil quando estiver renderizando em WEBGL, a canvas padrão não usa essa informação.
...
getURL__description__1 = Nota: ao utilizar o editor da p5, essa função retornará um objeto vazio, pois o objeto está embutido em uma iFrame. Ela funcionará corretamente se você visualizar o sketch utilizando o modo de apresentação, ou URLs compartilháveis.
getURL__returns = String: URL
getURLPath__description__0 = Retorna o caminho da URL atual como uma array.
getURLPath__description__1 = Nota: ao utilizar o editor da p5, essa função retornará um objeto vazio, pois o objeto está embutido em uma iFrame. Ela funcionará corretamente se você visualizar o sketch utilizando o modo de apresentação, ou URLs compartilháveis.
...
getURLParams__description__1 = Nota: ao utilizar o editor da p5, essa função retornará um objeto vazio, pois o objeto está embutido em uma iFrame. Ela funcionará corretamente se você visualizar o sketch utilizando o modo de apresentação, ou URLs compartilháveis.
...
setup__description__0 = A função <a href="#/p5/setup">setup()</a> é chaada quando o programa começa. É utilizada para definir propriedades do ambiente como o tamanho da canvas e cor do a cor de fundo da canvas. Também pode conter chamadas de carregamento de mídias (<a href="#/p5/loadImage">loadImage</a>, <a href="#/p5/loadJSON">loadJSON</a>, <a href="#/p5/loadFont">loadFont</a>, <a href="#/p5/loadStrings">loadStrings</a>, etc.). Só pode haver um bloco de <a href="#/p5/setup">setup()</a> por programa, não podendo ser chamado novamente após a execução inicial.
...
drawingContext__description__0 = A API do p5.js fornece muitas funcionalidades para a criação de gráficos, mas há algumas funcionalidades nativas do HTML5 Canvas que não são expostas pelo p5. Você ainda pode chamá-lo diretamente usando a variável  <code>drawingContext</code>, como no exemplo mostrado. Isso é o equivalente a chamar <code>canvas.getContext('2d');</code> ou <code>canvas.getContext('webgl');</code>. Veja isto <a href="https://developer.mozilla.org/en-US/docs/Web/API/CanvasRenderingContext2D"> referência para a API nativa do canvas</a> para possíveis funções de desenho que você pode chamar.
...
noLoop__description__4 = Observe que se a Canvas for redimensionada, <a href="#/p5/redraw"> redraw() </a> será chamado para atualizar o desenho, mesmo se <a href = "#/p5/noLoop">noLoop()</a> tiver sido chamado. Caso contrário, o desenho poderia ficar desconfigurado em relação ao tamanho da canvas até que <a href="#/p5/loop">loop()</a> fosse chamado. 
noLoop__description__5 = Utilize <a href="#/p5/isLooping">isLooping()</a> para verificar o atual estado de <code>loop()</code>.
...
pop__description__3 = No modo WEBGL, configurações de estilo adicionais são armazenadas. Elas são controlados pelas seguintes funções: <a href="#/p5/setCamera">setCamera()</a>, <a href="#/p5/ambientLight">ambientLight()</a>, <a href="#/p5/directionalLight">directionalLight()</a>, <a href="#/p5/pointLight">pointLight()</a>, <a href="#/p5/texture">texture()</a>, <a href="#/p5/specularMaterial">specularMaterial()</a>, <a href="#/p5/shininess">shininess()</a>, <a href="#/p5/normalMaterial">normalMaterial()</a> e <a href="#/p5/shader">shader()</a>.
...
redraw__description__4 = Além disso, você pode definir o número de redraws (o número de vezes que o código dentro de <a href="#/p5/draw">draw()</a> será executado) por chamada de método. Basta adicionar um inteiro como parâmetro único para o número de redraws.
...
p5__description__4 = Observe que a criação de instâncias como essa também permite que você tenha mais de um sketch do p5 em uma única página da web, pois cada um deles será empacotado (wrapped) com suas próprias variáveis de configuração. Você também pode usar <code>&lt;iframe&gt;</code> para ter vários sketches no modo global.
...
deviceShaken__description__1 = Por padrão, este valor é 30, mas pode ser alterado através da função <a href="#/p5/setShakeThreshold"><code>setShakeThreshold()</code></a>.
...
keyReleased__description__1 = Navegadores podem ter comportamentos diferentes relacionados a cada evento de tecla. Para previnir qualquer comportamento padrão, adicione <code>return false</code> ao fim do método.
...
keyIsDown__description__1 = Você pode verificar qualquer tecla através do seu código de tecla (key code), ou utilizar uma das contantes: <code>BACKSPACE</code>, <code>DELETE</code>, <code>ENTER</code>, <code>RETURN</code>, <code>TAB</code>, <code>ESCAPE</code>, <code>SHIFT</code>, <code>CONTROL</code>, <code>OPTION</code>, <code>ALT</code>, <code>UP_ARROW</code> (seta superior), <code>DOWN_ARROW</code> (seta inferior), <code>LEFT_ARROW</code> (seta esquerda), <code>RIGHT_ARROW</code> (seta direita).
...
specularColor__params__alpha = Número (opcional): valor de alpha
...
lightFalloff__params__Constante = Número: valor constante para determinar a queda (falloff)
