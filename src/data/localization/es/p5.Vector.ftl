...
dist__description__0 = Calculates the Euclidean distance between two points (considering a point as a vector object).
...
heading__description__0 = Calculate the angle of rotation for this vector(only 2D vectors). p5.Vectors created using <a src="#/p5/createVector">createVector()</a> will take the current <a = src="#/p5/angleMode">angleMode</a> into consideration, and give the angle in radians or degree accordingly.
...
angleBetween__description__0 = Calculates and returns the angle (in radians) between two vectors.
